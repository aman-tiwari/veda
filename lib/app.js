...
export default class GlslLivecoder {
  ...
  constructor(config: Config) {
    ...
    console.log(nm);
    this._manager = new PluginManager({ pluginsPath: nm,  lockStale: 1  });
  ...
  _tryGlslifyOrInstall(shader, filepath) {
    let out;
    try {
      out = glslify(shader, { basedir: path.dirname(filepath) });
      console.log('out:', out);
    } catch(e) {
      let catcher = /(Cannot find module)\s+\'(.+)\'\s+/g
      let message = e.message;
      
      let matches = catcher.exec(message);
      let moduleToInstall = matches[2].split('/')[0];
      console.log('installing: ' + moduleToInstall);
      return this._manager.install(moduleToInstall)
             .then(() => glslify(shader, { basedir: path.resolve(__dirname, '../node_modules') }))
             .then((sh) =>  { console.log(sh); return sh });
    }
